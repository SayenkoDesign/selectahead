...
{	
	...
	{ 
		...
		if($version!="0")
		{
			...
			if($version < "1.5.8" || $version > "2.0.9" )
			{
...
//		die("hihi");
	...
	#
	# function to return the store taxrates list so synch with QB inventory
	#
	//function getTaxRates($username,$password,$others)
//	{ 
//		global $wpdb;
//		$status=$this->auth_user($username,$password);
//		if($status!='0')
...
//		if($version > '3.7.8')
//		{
//			$region_data1 = $wpdb->get_results("SELECT  option_value  FROM ".wp_options." WHERE option_name = 'wpec_taxes_bands' ") ;
//			$region_data = unserialize($region_data1[0]->option_value);
//		}
//		else
//		{
...
//		}
//		
//		$TaxRates = new WG_TaxRates();
//		$TaxRates->setStatusCode('0');
//		$TaxRates->setStatusMessage('All Ok');
//		
//		if($region_data)
//		{ 
//			
//			if($version > '3.7.8')
//			{
//				foreach ($region_data as $iInfo) 
//				{
//					$TaxRate =new WG_TaxRate();
//					$TaxRate->setTaxClassID("");
//					$TaxRate->setTaxClassName(htmlentities($iInfo['name']));			
//					$TaxRate->setRateId(htmlentities(""));
//					$TaxRate->setRateName(htmlentities(""));
//					$TaxRate->setZone(htmlentities($iInfo1['region_code']));
//					$TaxRate->setRate(htmlentities($iInfo1['rate']));			
//					$TaxRates->setTaxRates($TaxRate->getTaxRate());	
//				}
//			
//			}
//			
...
//				
		...
		//unset($taxes);
//		return $this->response($TaxRates->getTaxRates());
//	} //getTaxRates
...
//		$datefrom = mktime(0,0,0,$mm,$dd,$yy); 
	...
	elseif(!$orderlist && $LastModifiedDate){
		...
		if(!isset($MaxOrderNoInBatch) || $MaxOrderNoInBatch=='')
        {
			//echo "SELECT t.term_id, p.ID FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON tt.term_id = t.term_id INNER JOIN $wpdb->term_relationships AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id INNER JOIN $wpdb->posts AS p ON p.ID = tr.object_id  WHERE tt.taxonomy IN ('shop_order_status') AND t.term_id IN (".QB_ORDERS_DOWNLOAD_EXCL_LIST.")AND  ".$str_date_filter."  AND p.post_type= 'shop_order' AND p.post_status!='trash' order by p.post_modified ASC ".(QB_ORDERS_PER_RESPONSE>0?"LIMIT 0, ".QB_ORDERS_PER_RESPONSE:'');
...
//			$Order->setTime("");
			...
			$Ship->setShipMethod($shipping_method?$shipping_method:$order->shipping_method_title);
			$Ship->setCarrier($order->shipping_method_title?$order->shipping_method_title:$order->shipping_method);
...
//				}
				...
				$to_unit = 'lbs';
				$weight_in_lbs = woocommerce_get_weight($_product['_height'][0], $to_unit);
				$Item->setWeight($weight_in_lbs);
				$Item->setFreeShipping('');
				$Item->setDiscounted('');
				$Item->setshippingFreight('');
