...
class WC_CSP_Condition_Cart_Shipping_Class extends WC_CSP_Condition {
	...
	public function __construct() {
		...
		$this->title                         = __( 'Shipping Class', WC_Conditional_Shipping_Payments::TEXT_DOMAIN );
		$this->supported_global_restrictions = array( 'shipping_countries', 'payment_gateways' );
	}
	...
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $data[ 'modifier' ] === 'in' && ! empty( $qualifying_products ) ) {
			...
			if ( count( $qualifying_products ) == 1 ) {
				...
				return sprintf( __( 'remove &quot;%s&quot; from your cart', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), current( $qualifying_products ) );
				...
				for ( $i = 1; $i < count( $qualifying_products ) - 1; $i++ ) {
					...
					$string = sprintf( __( '%1$s, &quot;%2$s&quot;', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $string, $qualifying_products[ $i ] );
				}

				/* translators: Used to stitch together product names - last name */
				$string = sprintf( __( '%1$s, and &quot;%2$s&quot;', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $string, end( $qualifying_products ) );

				return sprintf( __( 'remove %s from your cart', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $string );
			}
	...
	public function get_admin_fields_html( $index, $condition_index, $condition_data ) {
		...
		<div class="condition_modifier">
			<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][modifier]">
				<option value="in" <?php selected( $modifier, 'in', true ) ?>><?php echo __( 'in cart', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ); ?></option>
				<option value="not-in" <?php selected( $modifier, 'not-in', true ) ?>><?php echo __( 'not in cart', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ); ?></option>
			</select>
		</div>
		<div class="condition_value">
			<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][value][]" style="80%;" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select shipping classes&hellip;', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ); ?>">
				<?php
