...
class WC_CSP_Condition_Package_Category extends WC_CSP_Condition {
	...
	public function __construct() {
		...
		$this->title                         = __( 'Category', WC_Conditional_Shipping_Payments::TEXT_DOMAIN );
	...
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $data[ 'modifier' ] === 'in' && $contains_qualifying_products ) {
			...
			if ( count( $term_names ) == 1 ) {
				...
				return sprintf( __( 'remove all products in the &quot;%s&quot; category from your cart', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), current( $term_names ) );
				...
				for ( $i = 1; $i < count( $term_names ) - 1; $i++ ) {
					...
					$string = sprintf( __( '%1$s, &quot;%2$s&quot;', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $string, $term_names[ $i ] );
				}

					/* translators: Used to stitch together product category names - last name */
				$string = sprintf( __( '%1$s, and &quot;%2$s&quot;', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $string, end( $term_names ) );

				return sprintf( __( 'remove all products in the %s categories from your cart', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $string );
			}

		} elseif ( $data[ 'modifier' ] === 'not-in' && ! $contains_qualifying_products ) {
			return __( 'purchase a qualifying product', WC_Conditional_Shipping_Payments::TEXT_DOMAIN );
		}
	...
	public function get_admin_fields_html( $index, $condition_index, $condition_data ) {
		...
		<div class="condition_modifier">
			<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][modifier]">
				<option value="in" <?php selected( $modifier, 'in', true ) ?>><?php echo __( 'in package', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ); ?></option>
				<option value="not-in" <?php selected( $modifier, 'not-in', true ) ?>><?php echo __( 'not in package', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ); ?></option>
			</select>
		</div>
		<div class="condition_value">
			<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][value][]" style="width:80%;" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select categories&hellip;', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ); ?>">
				<?php
