...
class WC_Pos_Registers{
	...
	public function save_register(){
		...
		foreach (self::$register_detail_fields as $key => $value) {
			...
			$detail[$key] = $_POST['_register_' . $key];
		}
		foreach (self::$register_end_of_sale_fields as $key => $value) {
			$settings[$key] = $_POST['_register_' . $key];
		}
		$data['name']    = $_POST['_register_name'];

		$data['slug']    = wc_sanitize_taxonomy_name( stripslashes( $data['name']  ) );
		$data['outlet']  = $_POST['_register_outlet'];
		...
		if($id){
		  ...
		  $new_order = array(
		     ...
		     'post_type' => 'pos_temp_register_order'
		  );
	...
	function display_register_detail($id = 0){
		require_once( dirname(realpath(dirname(__FILE__) ) ).'/views/html-admin-registers.php' );
	}
  ...
  public function get_data($ids = ''){
    ...
    if( isset($_GET['s']) && !empty($_GET['s']) && $_GET['page'] == WC_POS()->id_registers ){
      $s = $_GET['s'];
