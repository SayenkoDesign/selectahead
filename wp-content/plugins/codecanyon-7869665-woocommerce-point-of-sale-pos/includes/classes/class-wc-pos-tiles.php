...
class WC_Pos_Tiles {
      ...
      $data['tile_style']       = $data['style'];
    	...
    	<form id="edit_wc_pos_outlets" class="validate" action="" method="post">
    		...
    		<input type="hidden" value="wc_pos_edit_update_tiles" name="action">
				...
				<table class="form-table">
					<tbody>
						<tr class="form-field form-required">
							...
							<td>
								<?php if($data['product_id']){  $product = get_product( $data['product_id'] ); ?>
									<select id="product_id" class="ajax_chosen_select_products" style="width: 349px;" name="product_id">
									 <option value="<?php echo $data['product_id']; ?>"><?php echo $product->get_formatted_name(); ?></option>
									</select>
								<?php 
                  $image = '';
                  $size = 'shop_thumbnail';
                  ...
                  if(!$image || $image == NULL) $image = wc_placeholder_img_src();
									...
									<select id="product_id" class="ajax_chosen_select_products" style="width: 294px;" name="product_id">
									 <option value=""><?php _e( 'Search for a product&hellip;', 'wc_point_of_sale' ); ?></option>
									</select>
								<?php
                $image = '';
                } ?>
						...
						<tr class="form-field tile_style_row">
							<th valign="top" scope="row">
								<label for="tile_style"><?php _e( 'Tile Style', 'wc_point_of_sale' ); ?></label>
							</th>
							<td>
								<ul class="wc-radios">
									<li>
				...
				<p class="submit">
					<input type="submit" name="add_new_tiles" id="submit" class="button button-primary" value="<?php _e( 'Save Your Tile', 'wc_point_of_sale' ); ?>"></p>
    	</form>
  	</div>
		<?php
	}
		    	...
		    	<div id="col-right">
		    		<div class="col-wrap">
						<div  style="clear:both"> </div>
							<form id="wc_pos_tiles_table" action="" method="post">
								<?php
                          ...
                          if ( $titles ) :
                              foreach ($titles as $title) :
                                  ...
                                  if($title->style == 'image'){
                                    ...
                                    ?>
                                    <td id="title_<?php echo $title->ID ?>" style="background: url('<?php echo $image; ?>') center no-repeat; background-size: contain; background-color: #fff;" class="title_product">
                                      <a style="color: #<?php echo $title->colour;?>;" data-id="<?php echo $product_id; ?>"></a>
                                    </td>
                                    <?php
                                  }else{ ?>
		    	...
		    	?>
		    		...
		    		<div class="col-wrap">
									...
									<div class="form-field">
										<label for="product_id"><?php _e( 'Product', 'wc_point_of_sale' ); ?></label>
										 ...
										 <?php if($product_id) {
										 		...
										 		<option value="<?php echo $product_id; ?>"><?php echo $product_data->get_formatted_name(); ?></option>
										 <?php 
	                    $image = '';
	                    $size = 'shop_thumbnail';
	                    if ( has_post_thumbnail( $product_id ) ) {
	                   ...
	                   $image = '';
									...
									<div class="form-field dafault_selection" <?php echo (!$default_selection || !$product_id) ? 'style="display: none;"' : ''; ?> >
										<label for="dafault_selection"><?php _e( 'Dafault Selection', 'wc_point_of_sale' ); ?></label>
										<?php if (!$product_id) { ?>
											<select name="dafault_selection" id="dafault_selection" ></select>
										<?php }else{ ?>
											<select name="dafault_selection" id="dafault_selection" class="show_dafault_selection">
												<option value="0" ><?php _e("No Default Selection", "wc_point_of_sale"); ?></option>;
												<?php
								        $args = array(
								            'post_type'      => array( 'product_variation' ),
	...
	public function get_data($ids = ''){
        ...
        $filter .= ' ORDER BY order_position ASC';
        $table_name = $wpdb->prefix . "wc_poin_of_sale_tiles";
