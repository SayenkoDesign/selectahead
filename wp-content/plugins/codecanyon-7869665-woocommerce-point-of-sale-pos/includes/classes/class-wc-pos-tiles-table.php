...
class WC_Pos_Tiles_Table extends WP_List_Table {
  private static $data;
  ...
  global $status, $page;
      ...
      parent::__construct( array(
          'singular'  => __( 'tiles_table', 'wc_point_of_sale' ),     //singular name of the listed records
          'plural'    => __( 'tiles_tables', 'wc_point_of_sale' ),   //plural name of the listed records
  ...
  function column_product( $item ) {
    ...
    if( $product->is_type( array( 'variable' ) ) ){
      if( $item['default_selection'] ){
        $product_variation = get_product( $item['default_selection'] );
    ...
    $name =  sprintf('<strong><a href="?page=%s&grid_id=%s&action=%s&id=%s">%s</a></strong>%s','wc_pos_tiles',$item['grid_id'],'edit', $item['ID'], $product->post->post_title, $variation_detail);
