...
if ( ! class_exists( 'WC_Pos_Settings' ) ) :

	/**
	 * WC_Pos_Settings
	 */
	include_once( WC()->plugin_path().'/includes/admin/class-wc-admin-settings.php' );
	class WC_Pos_Settings extends WC_Admin_Settings {
		...
		public static function get_settings_pages() {
			if ( empty( self::$settings ) ) {
				...
				$settings[] = include( dirname(realpath(dirname(__FILE__) ) ).'/settings/wc-pos-settings-general.php' );
				$settings[] = include( dirname(realpath(dirname(__FILE__) ) ).'/settings/wc-pos-settings-tax.php' );
				$settings[] = include( dirname(realpath(dirname(__FILE__) ) ).'/settings/wc-pos-settings-payment_methods.php' );
				$settings[] = include( dirname(realpath(dirname(__FILE__) ) ).'/settings/wc-pos-settings-layout.php' );
		...
		*/
		...
		public static function output() {
	      ...
	      if ( ! empty( $_GET['wc_message'] ) )
		    ...
		    include_once( dirname(realpath(dirname(__FILE__) ) ).'/views/html-admin-settings.php' );
