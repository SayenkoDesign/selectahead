...
class WC_Pos_Outlets{
	...
	public static $outlet_address_fields;
	public static $outlet_contact_fields;
	public static $outlet_social_fields;
	...
	public function init_form_fields($country = '') {
		...
		self::$outlet_address_fields = $filds;

		self::$outlet_contact_fields = array(
				'email' => array(
		...
		self::$outlet_social_fields = array(
				'twitter'       => array(
	...
	public function display_edit_form($id = 0) {
		...
		if($id){
			...
			self::init_form_fields($data['country']);	
		}
					...
					<tbody>
						<tr class="form-field form-required">
							...
							<td>
								...
								<?php } ?>
								<p class="description"><?php echo _e( 'The name of the outlet as it appears when opening a register.', 'wc_point_of_sale' ); ?><p>				
							</td>
						</tr>
						<?php 
							foreach ( self::$outlet_address_fields as $key => $field ) {
								if ( ! isset( $field['type'] ) )
									$field['type'] = 'text';
									$value = "";
						...
						<?php 
								foreach ( self::$outlet_contact_fields as $key => $field ) {
									if ( ! isset( $field['type'] ) )
										$field['type'] = 'text';
										$value = "";
										if($data[$key]){
						...
						<?php 
								foreach ( self::$outlet_social_fields as $key => $field ) {
									if ( ! isset( $field['type'] ) )
										$field['type'] = 'text';
										$value = "";
										if($data[$key]){
  ...
  <?php endif; ?>
	...
	public function save_outlet(){
		...
		$social = array();
		$data = array();
		foreach (self::$outlet_address_fields as $key => $value) {
			$contact[$key] = $_POST['_outlet_' . $key];
		}
		foreach (self::$outlet_contact_fields as $key => $value) {
			$social[$key] = $_POST['_outlet_' . $key];
		}
		foreach (self::$outlet_social_fields as $key => $value) {
			$social[$key] = $_POST['_outlet_' . $key];
		}		
		$data['name']    = $_POST['_outlet_name'];
	...
	public function get_data($ids = ''){
        ...
        if( isset($_GET['s']) && !empty($_GET['s']) && $_GET['page'] == WC_POS()->id_outlets  ){
          $s = $_GET['s'];
