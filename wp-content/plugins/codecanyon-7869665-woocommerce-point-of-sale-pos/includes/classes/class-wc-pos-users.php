...
class WC_Pos_Users {
	...
	public function display()
	{
		...
		<div class="wrap">
			...
			<p class="description"><?php _e( 'Below are the users who have an outlet assigned to them.', 'wc_point_of_sale' ); ?></p>
				<?php
  ...
  function is_logged_in($user_id) {
    ...
    $lock = explode( ':', $lock );
    $time = $lock[0];
    $user = isset( $lock[1] ) ? $lock[1] : $row->_edit_last;

    $time_window = apply_filters( 'wp_check_post_lock_window', 400 );
